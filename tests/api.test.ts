...
const app = createApp({ database: ':memory:', origin, githubToken: 'unused-local-test-token', githubAllowedRepos: [] });
...
describe('HTTP API and ownership boundaries', () => {
    before(async () => { await new Promise<void>(r => app.server.listen(0, '127.0.0.1', r)); const addr = app.server.address(); if (!addr || typeof addr === 'string')
        throw new Error('Missing test address'); base = `http://127.0.0.1:${addr.port}`; });
    ...
    it('rejects expired links', async () => { const r = await request(`/api/sessions/${session}/share`, 'POST', { hours: 1 }); app.store.db.prepare('UPDATE shares SET expires_at=0').run(); const token = r.value.url.split('/').at(-1); assert.equal((await request('/api/shared/' + token)).response.status, 404); });
    ...
    it('deletes owned sessions and dependent events', async () => { assert.equal((await request('/api/sessions/' + session, 'DELETE')).response.status, 200); assert.equal((await request('/api/sessions/' + session)).response.status, 404); assert.equal(app.store.db.prepare('SELECT count(*) n FROM events').get()?.n, 0); });
