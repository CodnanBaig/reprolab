...
export function createApp(options?: AppOptions & { store?: undefined }): { server: ReturnType<typeof createServer>; store: Store; handler: Handler };
export function createApp<S extends Persistence>(options: AppOptions & { store: S }): { server: ReturnType<typeof createServer>; store: S; handler: Handler };
export function createApp(options: AppOptions & { store?: Persistence } = {}) {
    const store = options.store ?? new Store(options.database ?? process.env.DATABASE_PATH ?? '.data/reprolab.sqlite');
    ...
    const requireUser = async (req: IncomingMessage): Promise<User> => {
        const user = (await store.userForToken(cookieValue(req.headers.cookie, 'repro_session')));
        if (!user)
            throw new HttpError(401, 'Sign in to continue.');
        return user;
    };
    const csrf = (req: IncomingMessage) => {
        if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method ?? 'GET') && req.headers.origin !== origin)
    ...
    async function body(req: IncomingMessage, limit = 3000000): Promise<Record<string, unknown>> {
        ...
        if (supplied !== undefined) {
            // Vercel may provide an already-parsed body; local Node streams remain supported.
            encoded = Buffer.isBuffer(supplied) ? supplied : Buffer.from(typeof supplied === 'string' ? supplied : JSON.stringify(supplied));
            if (encoded.length > limit) throw new HttpError(413, 'Request body exceeds the allowed limit.');
        } else {
            const chunks: Buffer[] = [];
            let size = 0;
            for await (const chunk of req) {
                size += chunk.length;
                if (size > limit) throw new HttpError(413, 'Request body exceeds the allowed limit.');
                chunks.push(chunk);
            }
            encoded = Buffer.concat(chunks);
        }
        let v;
        try { v = JSON.parse(encoded.toString('utf8')); }
        catch { throw new HttpError(400, 'Invalid JSON.'); }
        if (!v || typeof v !== 'object' || Array.isArray(v))
            throw new HttpError(400, 'Expected a JSON object.');
        return v;
    }
    async function issueCookie(res: ServerResponse, userId: string) { const token = randomToken(); (await store.db.prepare('INSERT INTO auth_sessions VALUES(?,?,?)').run(hash(token), userId, Date.now() + 7 * 86400000)); res.setHeader('Set-Cookie', `repro_session=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=604800${secure ? '; Secure' : ''}`); }
    async function detail(s: StoredSession, shared = false) {
        return { ...s, events: (await store.events(s.id)), notes: shared ? [] : (await store.db.prepare('SELECT * FROM notes WHERE session_id=? ORDER BY created_at').all(s.id)) };
    }
    const handler: Handler = async (req, res) => {
        ...
        try {
            ...
            if (path === '/api/health' && method === 'GET') {
                ...
                return json(res, 200, { ok: true, version: '0.1.0', storage: options.store ? 'remote-libsql' : 'sqlite' });
            }
            if (path === '/api/ingest') {
                if (method === 'OPTIONS') {
                ...
                const p = (await store.db.prepare('SELECT * FROM projects WHERE key_hash=?').get(hash(key))) as unknown as Project | undefined;
                if (!p)
                ...
                (await store.rateLimit(`ingest:${p.id}`, 60));
                ...
                const result = (await store.saveCapture(p.id, capture));
                return json(res, result.duplicate ? 200 : 201, result);
            }
            if (path.startsWith('/api/shared/') && method === 'GET') {
                const token = path.split('/').at(-1) ?? '';
                const row = (await store.db.prepare('SELECT s.* FROM sessions s JOIN shares sh ON sh.session_id=s.id WHERE sh.token_hash=? AND sh.expires_at>?').get(hash(token), Date.now())) as unknown as StoredSession | undefined;
                if (!row)
                    throw new HttpError(404, 'This share has expired or was revoked.');
                const { project_id, client_id, github_url, ...safe } = (await detail(row, true));
                void project_id;
                void client_id;
                void github_url;
            ...
            if (path.startsWith('/api/')) {
                ...
                if (['/api/auth/register', '/api/auth/login'].includes(path) && method === 'POST') {
                    (await store.rateLimit(`auth:${options.clientAddress?.(req) ?? req.socket.remoteAddress}`, 15, 15 * 60000));
                    ...
                    if (path.endsWith('register')) {
                        if (options.registrationEnabled === false) throw new HttpError(403, 'Registration is closed on this instance. Ask the operator for access.');
                        const name = requiredString(v.name, 'Name', 1, 60), id = randomUUID();
                        if ((await store.db.prepare('SELECT id FROM users WHERE email=?').get(email)))
                            throw new HttpError(409, 'That email is already registered.');
                        (await store.db.prepare('INSERT INTO users VALUES(?,?,?,?,?)').run(id, email, name, passwordHash(pass), Date.now()));
                        (await issueCookie(res, id));
                        return json(res, 201, { user: { id, email, name } });
                    }
                    const row = (await store.db.prepare('SELECT * FROM users WHERE email=?').get(email));
                    ...
                    (await issueCookie(res, String(row.id)));
                    return json(res, 200, { user: { id: row.id, email: row.email, name: row.name } });
                }
                if (path === '/api/auth/logout' && method === 'POST') {
                    (await store.db.prepare('DELETE FROM auth_sessions WHERE token_hash=?').run(hash(cookieValue(req.headers.cookie, 'repro_session'))));
                    res.setHeader('Set-Cookie', `repro_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0${secure ? '; Secure' : ''}`);
                    return json(res, 200, { ok: true });
                }
                const user = (await requireUser(req));
                if (path === '/api/projects' && method === 'GET') {
                    return json(res, 200, (await store.db.prepare('SELECT p.id,p.name,p.origins,p.repo,p.created_at,(SELECT count(*) FROM sessions s WHERE s.project_id=p.id) session_count FROM projects p WHERE owner_id=? ORDER BY created_at').all(user.id)).map(p => ({ ...p, origins: JSON.parse(String(p.origins)) })));
                }
                if (path === '/api/projects' && method === 'POST') {
                    const v = await body(req, 10000), name = requiredString(v.name, 'Project name', 1, 80), origins = parseOrigins(v.origins);
                    if (Number((await store.db.prepare('SELECT count(*) n FROM projects WHERE owner_id=?').get(user.id))?.n) >= 20)
                        throw new HttpError(400, 'A workspace supports up to 20 projects.');
                    const id = randomUUID(), key = randomToken();
                    (await store.db.prepare('INSERT INTO projects(id,owner_id,name,origins,key_hash,created_at) VALUES(?,?,?,?,?,?)').run(id, user.id, name, JSON.stringify(origins), hash(key), Date.now()));
                ...
                if (projectMatch) {
                    ...
                    if (action === 'rotate-key' && method === 'POST') {
                        ...
                        (await store.db.prepare('UPDATE projects SET key_hash=? WHERE id=?').run(hash(key), id));
                        return json(res, 200, { captureKey: key });
                    }
                    if (action === 'source-maps') {
                        if (method === 'GET')
                            return json(res, 200, (await store.db.prepare('SELECT id,asset,release,created_at FROM source_maps WHERE project_id=? ORDER BY created_at DESC').all(id)));
                        if (method === 'POST') {
                            ...
                            (await store.db.prepare('INSERT INTO source_maps(id,project_id,asset,release,data,created_at) VALUES(?,?,?,?,?,?) ON CONFLICT(project_id,asset,release) DO UPDATE SET data=excluded.data,created_at=excluded.created_at').run(sid, id, safeUrl(asset), release, JSON.stringify(map), Date.now()));
                    ...
                    if (!action && method === 'PATCH') {
                        ...
                        (await store.db.prepare('UPDATE projects SET name=?,origins=?,repo=? WHERE id=?').run(name, JSON.stringify(origins), repo, id));
                        return json(res, 200, { ok: true });
                    }
                    if (!action && method === 'DELETE') {
                        ...
                        (await store.db.prepare('DELETE FROM projects WHERE id=?').run(id));
                        return json(res, 200, { ok: true });
                    }
                }
                if (path === '/api/sessions' && method === 'GET') {
                    ...
                    (await store.ownerProject(project, user.id));
                    const search = (url.searchParams.get('search') ?? '').slice(0, 100), status = url.searchParams.get('status') ?? '';
                    const rows = (await store.db.prepare(`SELECT * FROM sessions WHERE project_id=? AND title LIKE ? AND (?='' OR status=?) ORDER BY created_at DESC LIMIT 250`).all(project, `%${search}%`, status, status));
                    const stats = (await store.db.prepare("SELECT count(*) total,coalesce(sum(error_count),0) errors,coalesce(sum(CASE WHEN status='resolved' THEN 1 ELSE 0 END),0) resolved,count(DISTINCT CASE WHEN error_count>0 THEN fingerprint END) groups FROM sessions WHERE project_id=?").get(project));
                ...
                if (match) {
                    const id = match[1]!, action = match[2], session = (await store.ownerSession(id, user.id));
                    if (!action && method === 'GET')
                        return json(res, 200, (await detail(session)));
                    if (!action && method === 'PATCH') {
                        const v = await body(req, 2000);
                        if (!['new', 'investigating', 'resolved', 'ignored'].includes(String(v.status)))
                            throw new HttpError(400, 'Invalid triage state.');
                        (await store.db.prepare('UPDATE sessions SET status=? WHERE id=?').run(String(v.status), id));
                        return json(res, 200, { ok: true });
                    }
                    if (!action && method === 'DELETE') {
                    ...
                    if (action === 'notes' && method === 'POST') {
                        ...
                        (await store.db.prepare('INSERT INTO notes VALUES(?,?,?,?)').run(randomUUID(), id, note, Date.now()));
                        return json(res, 201, { ok: true });
                    }
                    if (action === 'share' && method === 'POST') {
                        ...
                        (await store.db.prepare('INSERT INTO shares VALUES(?,?,?)').run(hash(token), id, Date.now() + hours * 3600000));
                        return json(res, 201, { url: `${origin}/#/share/${token}`, expiresAt: Date.now() + hours * 3600000 });
                    }
                    if (action === 'share' && method === 'DELETE') {
                        (await store.db.prepare('DELETE FROM shares WHERE session_id=?').run(id));
                        return json(res, 200, { ok: true });
                    }
                    if (action === 'export' && method === 'GET') {
                        const format = url.searchParams.get('format'), events = (await store.events(id));
                    ...
                    if (action === 'symbolicate' && method === 'POST') {
                        const errors = (await store.events(id)).filter(e => e.kind === 'error');
                        const positions = [];
                        for (const event of errors)
                            for (const line of String(event.data.stack ?? '').split('\n')) {
                                ...
                                if (!m)
                                    continue;
                                const map = (await store.db.prepare('SELECT data FROM source_maps WHERE project_id=? AND asset=? AND release=?').get(session.project_id, safeUrl(m[1]), session.release));
                                if (map)
                    ...
                    if (action === 'github' && method === 'POST') {
                        ...
                        (await store.db.prepare('UPDATE sessions SET github_url=? WHERE id=?').run(issue.html_url, id));
            ...
            const storageFailure = error instanceof Error && ['LibsqlError', 'TimeoutError', 'AbortError'].includes(error.name);
            const status = error instanceof HttpError ? error.status : storageFailure ? 503 : 500;
            if (status === 500)
                console.error('ReproLab request failed:', error instanceof Error ? error.name : 'UnknownError');
            if (!res.headersSent)
                json(res, status, { error: error instanceof HttpError ? error.message : storageFailure ? 'Storage is temporarily unavailable. Try again shortly.' : 'Something went wrong. Try again.' });
            else
                res.end();
        }
    };
